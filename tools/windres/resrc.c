...
run_cmd (char *cmd, const char *redir)
{
  ...
  if (redir_handle == -1)
    fatal ("can't open temporary file `%s': %s", redir,
	   strerror (errno));
  ...
  if (pid == -1)
	...
	{
	  fatal ("%s exited with status %d", cmd,
...
open_input_stream (char *cmd)
{
  if (istream_type == ISTREAM_FILE)
	...
	fatal ("can't execute `%s': %s", cmd, strerror (errno));

      cpp_pipe = fopen (cpp_temp_file, "r");;
      if (cpp_pipe == NULL)
	fatal ("can't open temporary file `%s': %s",
    ...
    {
      cpp_pipe = popen (cmd, "r");
      if (cpp_pipe == NULL)
	fatal ("can't popen `%s': %s", cmd, strerror (errno));
      if (verbose)
	fprintf (stderr, "Using popen to read preprocessor output\n");
    }

  atexit (close_input_stream);
  return cpp_pipe;
}
	      ...
	      const char *preprocargs, int language, int use_temp_file)
{
  ...
  if (preprocessor)
    {
      cmd = malloc (strlen (preprocessor)
		     + strlen (preprocargs)
		     + strlen (filename)
		     + 10);
  ...
  else
    {
      ...
      cmd = malloc (strlen (program_name)
		     + strlen (preprocessor)
		     + strlen (preprocargs)
		     + strlen (filename)
#ifdef HAVE_EXECUTABLE_SUFFIX
	       ...
	       const char *filename)
{
  ...
  e = open_file_search (filename, "rb", "bitmap file", &real_filename);
	       ...
	       const char *filename)
{
  ...
  e = open_file_search (filename, "rb", "cursor file", &real_filename);
  ...
  icondirs = (struct icondir *) malloc (count * sizeof *icondirs);
	     ...
	     const char *filename)
{
  ...
  e = open_file_search (filename, "rb", "font file", &real_filename);
	     ...
	     const char *filename)
{
  ...
  e = open_file_search (filename, "rb", "icon file", &real_filename);
		     ...
		     const char *filename)
{
  ...
  e = open_file_search (filename, "rb", "messagetable file",
			&real_filename);
