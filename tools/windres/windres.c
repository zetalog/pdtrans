...
#define REPORT_BUGS_TO  "zhengl@sh.necas.nec.com.cn"
		  ...
		  char **real_filename)
{
  ...
  if (errno == ENOENT)
      ...
      for (d = include_dirs; d != NULL; d = d->next)
	{
	  ...
	  n = (char *) malloc (strlen (d->dir) + strlen (filename) + 2);
...
sort_resources (struct res_directory *resdir)
{
  ...
  /* This is a recursive routine, so using malloc is probably better
     than alloca.  */
  a = (struct res_entry **) malloc (c * sizeof (struct res_entry *));
...
format_from_name (const char *name, int exit_on_error)
{
  ...
  if (m->name == NULL && exit_on_error)
    {
      non_fatal ("unknown format type `%s'", name);
...
{
  fprintf (stream, "Usage: %s [option(s)] [input-file] [output-file]\n",
	   program_name);
  ...
  while ((c = getopt_long (argc, argv, "f:i:l:o:I:J:O:F:D:U:rhHvV", long_options,
	...
	case 'f':
	  /* For compatibility with rc we accept "-fo <name>" as being the
	     ...
	     line switches.  */
	  if (*optarg != 'o')
	    fatal ("invalid option -f\n");
	  optarg++;
	  if (* optarg == 0)
	    {
	      if (optind == argc)
		fatal ("No filename following the -fo option.\n");
	      optarg = argv [optind++];
	    }
	  /* Fall through.  */
	  ...
	  if (preprocargs == NULL)
	    {
	      ...
	      preprocargs = malloc (strlen (quotedarg) + 3);
	  ...
	  else
	    {
	      ...
	      quotedarg = quot (optarg);
	      n = malloc (strlen (preprocargs) + strlen (quotedarg) + 4);
	...
	case 'I':
	  ...
	  if (input_format_tmp != RES_FORMAT_UNKNOWN)
	    {
	      fprintf (stderr, "Option -I is deprecated for setting the input format, please use -J instead.\n");
...

	  {
	    ...
	    n = (struct include_dir *) malloc (sizeof *n);
	    n->next = NULL;
	    n->dir = optarg;
