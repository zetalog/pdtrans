...
static void write_res_bin
  (const struct res_resource *, const struct res_id *,
	   const struct res_id *, const struct res_res_info *);

static void write_res_id (const struct res_id *);
static void write_res_info (const struct res_res_info *);
static void write_res_data (const void *, size_t, int);
static void write_res_header
  (unsigned long, const struct res_id *, const struct res_id *,
	   const struct res_res_info *);

static int read_resource_entry (void);
static void read_res_data (void *, size_t, int);
static void read_res_id (struct res_id *);
static unichar *read_unistring (int *);
static void skip_null_resource (void);

static unsigned long get_id_size (const struct res_id *);
static void res_align_file (void);

static void
  res_add_resource
  (struct res_resource *, const struct res_id *,
	   const struct res_id *, int, int);

void
  res_append_resource
  (struct res_directory **, struct res_resource *,
     ...
     int *len;
{
  ...
  *len = 0;
  ...
  p = s = (unichar *) malloc (sizeof (unichar) * 256);
  do
    {
