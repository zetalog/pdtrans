...
if (@ARGV < 0) {
    println("Usage: perl pd_replace.pl path");
    exit;
}
...
if (-d $path) {
	pdr_replace_folder($path);
} elsif (-f $path) {
	&pdr_replace_file($path);
} else {
    error(__LINE__, __FILE__, "$path is not a folder or file");
}

################################################
## Recursively replace folder
################################################

sub pdr_replace_folder {
	my ($folder) = @_;

	recursive_call($folder, "pdr_replace_file");
}
...
sub pdr_replace_file {
	my ($original) = @_;

	my ($name, $type) = split /\./, $original;
	my $target = $original . ".pdr";
	my $resource = $original . ".csv";

	my $request = "pd_" . $type . ".pl";

	eval { require $request; };
	if ($@) {
		warning(__LINE__, __FILE__, "Unable to find required $request module");
		return;
	}

	my @contents = &readfile($original);
	my %resouorces = &pdr_load_resource($resource);
	$SUB = $type . _replace_contents;
	my @results = &$SUB(@contents, %resources);
	&writefile($target, @results);
}
...
sub pdr_load_resource {
	my ($resfile) = @_;
	my %table, $line;

	my @contents = &readfile($resfile);
