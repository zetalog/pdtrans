...
{
    js_process_t process = NULL;

    process = malloc(sizeof (struct _js_process_t));
    if (process)
    {
        process->language = LANGUAGE_JS;
        process->scanner = (pdt_scanner_t)pdt_scan_js_word;
        process->parser = (pdt_parser_t)pdt_parse_js_word;
        process->destroyer = (pdt_destroyer_t)pdt_destroy_js_processor;

        process->to_csv = (pdt_convert_t)pdt_js_to_csv;
        process->from_csv = (pdt_convert_t)pdt_js_from_csv;
...
void pdt_parse_js_word(js_process_t process, char *word, FILE *stream)
{
        ...
        (*word == '\'' && *(word + strlen(word) - 1) == '\''))
    {
        if (pdt_must_localize(word))
        {
            ...
            {
                csv_in = pdt_js_to_csv(process, word);
                ...
                if (csv_in)
                {
                    ...
                    if (csv_out)
                    {
                        if (process->replace_type & PDT_REPLACE_TRANSLATE)
                            process->replace_current = pdt_js_from_csv(process, csv_out);
                        else
...
int pdt_scan_js_word(js_process_t process, char **bufptr, size_t *n,
                     int *word_begin, FILE *stream)
{
    ...
    assert(process && process->language == LANGUAGE_JS && bufptr && n && word_begin && stream);
...
char *pdt_js_to_csv(js_process_t process, const char *word_in)
{
    ...
    if (!word_out) {
        pdt_report_error(PDT_REPORT_ERROR, "Cannot malloc for %s in js_to_csv.\n", word_in);
        return NULL;
    }
...
char *pdt_js_from_csv(js_process_t process, const char *word_in)
{
    ...
    if (!word_out) {
        pdt_report_error(PDT_REPORT_ERROR, "Cannot malloc for %s in js_from_csv.\n", word_in);
        return NULL;
    }
...
pdt_plugin_t pdt_js_plugin = {
    ".js",
    pdt_create_js_processor,
    pdt_escape_asp_path,
    0,
    "japanese",
    "english",
